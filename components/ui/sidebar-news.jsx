...
import { useMediaQuery } from "@/hooks/use-mouse-position-ref";
...
export function News({
  ...
  const [dismissedNews, setDismissedNews] = React.useState([]);
      ...
      <div className="relative size-full">
        ...
        {showCompleted && !cardCount && (
          <div
            ...
            <div
              className="animate-fade-in absolute inset-0 rounded-lg border border-neutral-300 [animation-delay:2.3s] [animation-direction:reverse] [animation-duration:0.2s]" />
            <AnimatedLogo className="w-1/3" />
            <span
              className="animate-fade-in text-xs font-medium text-muted-foreground [animation-delay:2.3s] [animation-direction:reverse] [animation-duration:0.2s]">
  ...
  return (
    (<Card
      ...
      <div className={cn(hideContent && "invisible")}>
        ...
        <div
          className="relative mt-3 aspect-[16/9] w-full shrink-0 overflow-hidden rounded border bg-muted">
          {image && (
            <Image
              src={image}
              alt=""
